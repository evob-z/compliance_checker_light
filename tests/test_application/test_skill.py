...
class TestComplianceSkillInit:
    ...
    def test_init_with_default_config(
        self, mock_image_parser, mock_docx_parser, mock_pdf_parser,
        mock_use_case, mock_engine, mock_init_app
    ):
        """测试使用默认配置初始化"""
        # 设置 Mock
    ...
    @patch("src.application.skill.ProjectCheckUseCase")
    @patch("src.application.skill.PDFParser")
    def test_init_engine_config(
        self, mock_pdf_parser, mock_use_case, mock_engine, mock_init_app
    ):
        """测试引擎并发配置"""
        mock_registry = MagicMock()
        mock_container = MagicMock()
...
class TestComplianceSkillCheck:
    ...
    async def test_check_success_all_pass(self, mock_execution_result_all_pass):
        ...
        self.mock_use_case.execute = AsyncMock(return_value={
            "success": True,
