...
class MockExecutionResult:
    ...
    def __init__(self, success=True, has_invoice=True, date_valid=True, has_seal=True):
        ...
        # 1. 完整性检查 - 发票文件是否存在
        if has_invoice:
            project_results.append(CheckResult(
        ...
        if has_invoice:
            ...
            if date_valid:
                doc_results.append(CheckResult(
                    ...
                    details={
                        ...
                        "documents": [{
                            ...
                            "has_validity": True,
                            "has_sign_date": True,
                            "validity": {"value": 1, "unit": "years", "is_permanent": False},
                ...
                doc_results.append(CheckResult(
                    ...
                    details={
                        ...
                        "documents": [{
                            ...
                            "has_validity": True,
                            "has_sign_date": True,
                            "validity": {"value": 1, "unit": "months", "is_permanent": False},
                            "sign_date": "2026-03-10",
...
class TestInvoiceCheckEndToEnd:
    ...
    async def test_invoice_check_all_pass(self, tmp_path):
        ...
        with patch("src.application.skill.initialize_app") as mock_init:
            ...
            mock_container.llm_client.generate_yaml = AsyncMock(return_value={
                "checklist": {
    ...
    async def test_invoice_check_date_overdue(self, tmp_path):
        ...
        assert "日期" in issues_desc or "时效" in issues_desc or "过期" in issues_desc or "有效期" in issues_desc
    ...
    async def test_invoice_check_missing_seal(self, tmp_path):
        ...
        issues_desc = result["issues_description"]
        assert "印章" in issues_desc or "公章" in issues_desc or "视觉" in issues_desc or "未检测" in issues_desc or "未发现" in issues_desc
    ...
    async def test_invoice_check_path_not_found(self):
        ...
        with pytest.raises(FileNotFoundError) as exc_info:
            await use_case.execute(
                project_path="/nonexistent/path/to/invoice",
                requirements="检查是否有发票文件"
            )
...
class TestInvoiceCheckResultFormat:
    ...
    async def test_result_structure(self, tmp_path):
        ...
        # 验证必需字段
        required_fields = ["success", "summary", "issues_count", "issues", 
                          "issues_description", "messages", "execution_time"]
