...
class ComplianceChecker(BaseChecker):
    ...
    async def check(
        ...
        if not documents:
            return CheckResult(
                check_type=self.name, status=CheckStatus.PASS, message="没有文档需要检查合规性", details={}
            )
        ...
        try:
            ...
            for doc_comp in result.details:
                ...
                doc_details.append({"document_name": doc_comp.document_name, "checks": checks})

            details = {"documents": doc_details, "points_checked": points}
