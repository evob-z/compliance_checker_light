...
class DeclarativeCheckEngine:
    ...
    def __init__(self, registry: CheckerRegistry = None, concurrency_config: Dict[str, Any] = None):
    ...
    async def execute(self, documents: List[Document], checklist: Checklist) -> ExecutionResult:
        """
        执行声明式检查流程
    ...
    def _build_check_plan(self, documents: List[Document], checklist: Checklist) -> List[CheckTask]:
        """
        根据清单构建检查计划
    ...
    def _match_document_to_checklist(self, doc: Document, checklist: Checklist) -> Optional[Dict]:
        """
        匹配文档到清单项
    ...
    async def _execute_parallel(
        self, check_plan: List[CheckTask], checklist: Checklist
            ...
            if isinstance(checker, UnavailableChecker):
                try:
                    ...
                    result = CheckResult(
                        ...
                        details={"document": task.doc_name, "contact": "项目负责人", "status": "开发中"},
                    )
                return task, result
    ...
    def _aggregate_results(
        self, results: List[tuple[CheckTask, CheckResult]], plan: List[CheckTask]
    ) -> ExecutionResult:
        """汇总所有检查结果"""

        summary = {"total": len(results), "passed": 0, "failed": 0, "unavailable": 0, "errors": 0}
