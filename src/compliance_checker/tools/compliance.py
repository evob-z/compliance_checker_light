...
class TextComplianceChecker:
    ...
    def check_seal(self, text: str) -> ComplianceCheckItem:
        ...
        return ComplianceCheckItem(
            point="公章", found=False, status=CheckStatus.MISSING, message="文本中未发现公章相关关键词，建议进行视觉检查"
        )
    ...
    def check_signature(self, text: str) -> ComplianceCheckItem:
        ...
        return ComplianceCheckItem(
            point="签字", found=False, status=CheckStatus.MISSING, message="文本中未发现签字相关关键词，建议进行视觉检查"
        )

    def check_document_number(
        self, text: str, pattern: Optional[str] = None
    ) -> ComplianceCheckItem:
        """
        检查文件编号
    ...
    def check_date(self, text: str) -> ComplianceCheckItem:
        ...
        return ComplianceCheckItem(
            point="日期", found=False, status=CheckStatus.WARNING, message="未发现明确的日期信息"
        )
    ...
    async def _get_document_screenshot(self, pdf_path: str) -> Optional[str]:
        ...
        try:
            ...
            screenshot_path = os.path.join(
                temp_dir, f"compliance_check_{os.path.basename(pdf_path)}.png"
            )
...
class ComplianceChecker:
    ...
    async def check_with_rules(
        self, document: Document, compliance_points: List[CompliancePoint]
    ) -> DocumentCompliance:
        """
        根据指定的合规要点进行检查
            ...
            if point.check_method in (CheckMethod.VISUAL, CheckMethod.BOTH):
                ...
                if self.use_visual and self.visual_checker.is_available():
                    if point.point in ["公章", "印章"]:
                        ...
                        visual_check_item = await self.visual_checker.check_signature_visual(
                            document
                        )
                    ...
                    if check_item:
                        ...
                        check_item = ComplianceCheckItem(
                            ...
                            + (
                                " (QWEN_API_KEY未配置)"
                                if not self.visual_checker.is_available()
                                else ""
                            ),
        ...
        return DocumentCompliance(
            document_name=document.name, file_path=document.path, checks=checks
        )
    ...
    async def check_document(
        self, document: Document, checklist: Optional[Checklist] = None
    ) -> DocumentCompliance:
        """
        检查单个文档的合规性
        ...
        if checklist:
            for req_doc in checklist.required_documents:
                # 匹配文档
                is_match = req_doc.name in document.name or any(
                    alias in document.name for alias in req_doc.aliases
                )
            ...
            for p in self.points:
                ...
                if p in ["公章", "印章", "章"]:
                    custom_points.append(
                        CompliancePoint(point=p, required=True, check_method=CheckMethod.VISUAL)
                    )
                elif p in ["签字", "签名", "签署"]:
                    custom_points.append(
                        CompliancePoint(point=p, required=True, check_method=CheckMethod.VISUAL)
                    ...
                    custom_points.append(
                        CompliancePoint(point=p, required=True, check_method=CheckMethod.TEXT)
                    )
    ...
    async def check(
        ...
        return ComplianceResult(status=status, details=details)
