...
class DateExtractor:
    ...
    def extract_document_dates(self, document: Document) -> Dict[str, Optional[str]]:
        ...
        # 然后使用通用日期提取
            ...
            # 优先选择最精确的日期
            valid_dates = [d for d in extracted["issue_dates"] 
        ...
        if extracted["valid_from_dates"]:
            # 优先选择最精确的日期
            valid_dates = [d for d in extracted["valid_from_dates"] 
                          if not (d.month == 12 and d.day == 31) and d.day != 31]
            if valid_dates:
        ...
        if extracted["valid_to_dates"]:
            ...
            # 按日期分组，选择有具体日期的
            valid_dates = [d for d in extracted["valid_to_dates"] 
                          if not (d.month == 12 and d.day == 31) and d.day != 31]
            if valid_dates:
...
class TimelinessChecker:
    ...
    def _check_date_coverage(self, 
                             ...
                             validity_description: Optional[Dict] = None) -> Tuple[CheckStatus, str]:
        """
        检查日期是否覆盖项目周期
        ...
        if covers_end and covers_start:
            ...
            return CheckStatus.EXPIRED, f"有效期至 {valid_to}，未覆盖项目结束时间 {project_end.strftime('%Y-%m-%d')}"
    ...
    def _check_issue_date(self, 
                         ...
                         rule: ValidityRule) -> Tuple[CheckStatus, str]:
        """检查签发日期是否符合规则"""
        if not issue_date:
    ...
    def check_document(self, document: Document, validity_rule: Optional[ValidityRule] = None) -> TimelinessDetail:
        """
        检查单个文档的时效性
        ...
        if validity_rule:
            ...
            if validity_rule.issue_after or validity_rule.issue_before:
                issue_status, issue_message = self._check_issue_date(dates.get("issue_date"), validity_rule)
    ...
    def check(self, documents: List[Document], checklist: Optional[Checklist] = None) -> TimelinessResult:
        """
        执行时效性检查
