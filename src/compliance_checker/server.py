...
async def run_compliance_check(
    ...
    engine = DeclarativeCheckEngine(registry=registry, concurrency_config=concurrency_config)
...
async def list_available_checkers() -> dict:
    ...
    registry = get_initialized_registry()
    return {"available": registry.list_available(), "all_status": registry.list_all()}


@mcp.tool()
async def check_with_description(
    project_path: str, requirements: str, project_period: Optional[Dict[str, str]] = None
) -> dict:
    """
    自然语言合规审查（Skill 接口）
    ...
    try:
        ...
        result = await skill.check(
            project_path=project_path, requirements=requirements, project_period=project_period
        )
        return result
    except FileNotFoundError as e:
        return {"success": False, "error": f"路径错误: {str(e)}", "error_type": "file_not_found"}
    except ValueError as e:
        return {"success": False, "error": f"输入错误: {str(e)}", "error_type": "value_error"}
    except Exception as e:
        return {"success": False, "error": f"执行错误: {str(e)}", "error_type": type(e).__name__}


async def validate_checklist(checklist_yaml: str) -> dict:
    """
    验证清单 YAML 的合法性
...
async def get_checklist_generation_prompt(
    user_description: str, include_examples: bool = False
) -> dict:
    """
    ...
    return {"prompt": prompt, "usage": "将此 prompt 发送给 LLM，获取 YAML 输出后可用 validate_checklist 验证"}
...
async def parse_documents_tool(
    file_paths: Optional[List[str]] = None, extract_options: Optional[Dict[str, Any]] = None
) -> dict:
    """
    解析项目文档，提取文字和元数据。
...
async def check_compliance_tool(
    ...
    result = await check_compliance(
        documents=doc_objects, checklist=checklist_obj, rules=rules, use_visual=use_visual
    )
...
async def generate_report_tool(
    ...
    """
    return {"status": "error", "error": "PDF 生成功能已移除，请使用 check_with_description 工具获取文本格式的检查结果"}
