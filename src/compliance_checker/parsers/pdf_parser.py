...
class PDFParser:
    ...
    def _extract_with_ocr(self, file_path: str, total_pages: int) -> List[PageContent]:
        ...
        if ocr_engine is None:
            ...
            return [PageContent(page_num=i, text="", has_text_layer=False) 
        ...
        try:
            ...
            # OCR失败时返回空内容
            pages_content = [PageContent(page_num=i, text="", has_text_layer=False) 
                           for i in range(total_pages)]
        
        return pages_content
    
    def _extract_dates_from_content(self, metadata: DocumentMetadata, 
