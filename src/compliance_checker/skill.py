...
class ComplianceSkill:
    ...
    async def check(
        self, project_path: str, requirements: str, project_period: Optional[Dict[str, str]] = None
    ) -> Dict[str, Any]:
        """
        执行合规审查
        ...
        try:
            checklist_data = await async_generate_checklist_with_period(
                user_description=requirements, project_period=project_period
            )
    ...
    async def _parse_single_document(self, file_path: str) -> Optional[Document]:
        ...
        try:
            if ext == ".pdf":
                ...
                return await asyncio.get_event_loop().run_in_executor(None, parser.parse, file_path)
    ...
    async def check_with_generated_checklist(
        self, project_path: str, checklist_yaml: str
    ) -> Dict[str, Any]:
        """
        使用已生成的 YAML 清单执行检查
